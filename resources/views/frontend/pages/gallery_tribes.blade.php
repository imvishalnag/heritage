...
<section class="xs-section-padding">
	<div class="container">
		@if( count($gallery_image) == 0 )
	        <div class="text-center"><span class="text-danger">Sorry! No data found</span>
	        </div>
	     @endif
		<div class="row mb-2">
             @foreach( $gallery_image as $image )
				<div class="col-lg-4 col-md-6">
					<div class="xs-box-shadow xs-single-journal xs-mb-30">
						<a href="{{route('gallery.state.tribe.individual',['state'=>encrypt( $image->state), 'tribe'=>encrypt( $image->tribe)])}}">
							<div class="entry-thumbnail main_gallery_hover" style="max-height: 220px; overflow: hidden;">
								<img class="lazy" src="{{asset('loader.gif')}}" data-src="{{asset('assets/gallery/tribe/'.$image->file.'')}}" data-srcset="{{asset('assets/gallery/tribe/'.$image->file.'')}}" alt="">
								<span class="text-uppercase">{{$image->tribe}}</span>
